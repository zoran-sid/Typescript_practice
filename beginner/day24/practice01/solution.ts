...
function importTasks(text: string): ImportResult {
  ...
  if (!Array.isArray(parsed)) return { ok: false, message: "Task data is invalid" };
  // 回调关系：filter 把每一项交给 isStudyTask，只保留验证通过的任务。
...
// 调用关系：JSON 文字 -> importTasks -> ImportResult -> ok 分支 -> 输出与汇总。
