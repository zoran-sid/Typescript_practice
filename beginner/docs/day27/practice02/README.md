...
课程工具要在网页搜索框和命令行脚本中复用，同时还要接收服务端返回的课程标题。边界分别来自浏览器事件、类型未知的请求响应和字符串参数；任何一处读取错误都可能让工具只能在单一环境运行。你需要把三个入口转换成可测试的数据，并分别输出输入内容、验证后的标题和解析出的课程天数。
...
```text
输入事件 ──> bindSearch ──> SearchCallback ──> query
