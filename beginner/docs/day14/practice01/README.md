...
```text
类型模块 ── import type ──> Student 类型
数据模块 ── import ──────> student + scores
...
> **先看清输出标点：** 本题故意同时出现两种括号：`（21 课）` 使用中文全角括号，`(beginner)` 使用英文半角括号。代码语法里的 `()` 和 `:` 仍必须使用英文半角符号。运行器不会因显示文字中的括号或冒号全半角差异判你失败，但其他文字、数字和顺序仍要一致。
