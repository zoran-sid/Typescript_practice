...
function describeTask(task: StudyTask): string {
  ...
  switch (task.status) {
    ...
    case "completed": {
      // score 可能缺席；?? 不会误把 0 分当作“待评分”。
      const scoreText = task.score ?? "待评分";
      return `已完成：${task.title}（${scoreText}${task.score === undefined ? "" : " 分"}）`;
    }
    case "failed":
      return `失败：${task.title}（${task.reason}）`;
    default:
      // 新增状态却忘记补 case 时，task 将不再是 never。
...
for (const task of tasks) {
  // 调用关系：task -> describeTask(task) -> description -> console.log。
